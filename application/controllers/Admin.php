...
//--Schedule
   ...
   function teacher_show(){
    ...
    function e_student($nis){
        ...
        $data['admin']=$this->AdminModel->get_data_user($this->session->userdata('id'));
        $data['class']=$this->AdminModel->get_class(); 
        $data['student']=$this->AdminModel->get_data_students($nis);
        $this->load->view('pages/Admin/student_edit', $data);
    }

    function edit_student($nis){
        if(!$this->session->userdata('id') || $this->session->userdata('status')!='admin'){
			redirect(base_url());
        }
        ...
        $update = $this->AdminModel->update_data('nis','student',$data_update, $nis);

        if($update){
            $this->session->set_flashdata('alert', array('message' => 'Student NIS: '.$nis.' Edited!','class' => 'success'));
